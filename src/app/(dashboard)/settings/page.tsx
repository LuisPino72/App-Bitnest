...
export default function ExportPage() {
  const { referrals, getActiveReferrals, getTotalCommission } = useFirebaseReferrals();
  const { investments, getActiveInvestments } = useFirebasePersonalInvestments();
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        <div className="bg-white p-6 rounded-lg border border-gray-200">
          ...
          <div className="text-sm text-gray-600">
            Total invertido:{" "}
            {formatCurrency(
              getActiveInvestments().reduce((sum: number, inv: PersonalInvestment) => sum + inv.amount, 0)
            )}
          </div>
        </div>
      </div>
